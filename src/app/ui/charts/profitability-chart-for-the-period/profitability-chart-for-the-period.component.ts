...
@Component({
  selector: 'app-profitability-chart-for-the-period',
  templateUrl: './profitability-chart-for-the-period.component.html',
  styleUrls: ['./profitability-chart-for-the-period.component.less']
})
export class ProfitabilityChartForThePeriodComponent implements OnInit {
  @Output() changeDate = new EventEmitter();

  @Input() defaultStart: string;
  @Input() defaultEnd: string;
  @Input() loading: boolean;
  @Input() isYieldDataSuccess: boolean;
  ...
  private initChat(value: [number, number][]) {
    ...
    if (mod === 'day') {
      value.forEach((data) => {
        ...
        data[1] = +((data[1] * 100) / 100).toFixed(2);
